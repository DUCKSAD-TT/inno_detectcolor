...
class HardwareNode:
    def __init__(self, root: ctk.CTk):
        self.root = root
        self.root.title("Arduino Node — Servo Driver")
        self.root.geometry("480x1090")
        ...
        # Adjustable Ease-out parameters
        self.use_ease       = True
        self.sweep_tick     = 0.004    # Default sweep tick time (0.002s = 2ms)
        self.kp             = 0.25     # เพิ่มความเร็วในการตอบสนองชะลอตัวช่วงปลาย (Ease)
        self.min_step       = 0.1      # เพิ่มขนาดก้าวขั้นต่ำสุดเพื่อให้เข้าพิกัดปลายทางได้รวดเร็วขึ้นมาก
        self.s2_kp          = 0.25     # ความเร็วชะลอตัวของ Servo 2 
        self.s2_min_step    = 0.6      # ขนาดก้าวขั้นต่ำสุดของ Servo 2
        self.s_speed_ratios = [1.0, 0.8, 1.0, 1.0]  # Speed multipliers for S1, S2, S3, S4
    ...
    def _build_ui(self):
        ...
        # Checkbox for using Ease-out
        self.ease_chk_var = tk.BooleanVar(value=True)
        self.ease_chk = ctk.CTkCheckBox(
            speed_frame, text="Use Ease-out (Decelerate near target)",
            variable=self.ease_chk_var,
            font=("Tahoma", 14, "bold"),
            command=self._on_ease_toggle
        )
        self.ease_chk.pack(anchor="w", padx=12, pady=4)
        ...
        self.sweep_step_slider = ctk.CTkSlider(
            step_row, from_=0.5, to=90.0, number_of_steps=179,
            variable=self.sweep_step_var,
            command=self._on_sweep_step_change
        )
        self.sweep_step_slider.pack(fill="x", expand=True, padx=8)

        # --- General Ease-out tuning ---
        gen_frame = ctk.CTkFrame(speed_frame, fg_color="transparent")
        gen_frame.pack(fill="x", padx=8, pady=2)
        
        # S1, S3, S4 kp
        row_kp = ctk.CTkFrame(gen_frame, fg_color="transparent")
        row_kp.pack(fill="x", pady=1)
        ctk.CTkLabel(row_kp, text="Gen kp (Ease):", font=("Tahoma", 14)).pack(side="left")
        self.kp_lbl = ctk.CTkLabel(row_kp, text=f"{self.kp:.2f}", font=("Consolas", 14, "bold"))
        self.kp_lbl.pack(side="right", padx=4)
        self.kp_slider = ctk.CTkSlider(row_kp, from_=0.05, to=1.50, number_of_steps=145,
                                       command=self._on_kp_change)
        self.kp_slider.set(self.kp)
        self.kp_slider.pack(fill="x", expand=True, padx=8)

        # S1, S3, S4 min_step
        row_min = ctk.CTkFrame(gen_frame, fg_color="transparent")
        row_min.pack(fill="x", pady=1)
        ctk.CTkLabel(row_min, text="Gen min step:", font=("Tahoma", 14)).pack(side="left")
        self.min_step_lbl = ctk.CTkLabel(row_min, text=f"{self.min_step:.1f}°", font=("Consolas", 14, "bold"))
        self.min_step_lbl.pack(side="right", padx=4)
        self.min_step_slider = ctk.CTkSlider(row_min, from_=0.1, to=10.0, number_of_steps=99,
                                             command=self._on_min_step_change)
        self.min_step_slider.set(self.min_step)
        self.min_step_slider.pack(fill="x", expand=True, padx=8)

        # --- Servos Individual Speed Limits (Ratios) ---
        s_title_lbl = ctk.CTkLabel(speed_frame, text="Servos Speed Limits (Max Speed %)", font=("Tahoma", 15, "bold"))
        s_title_lbl.pack(anchor="w", padx=8, pady=(4, 0))

        s_ratios_frame = ctk.CTkFrame(speed_frame, fg_color="transparent")
        s_ratios_frame.pack(fill="x", padx=8, pady=2)

        self.s_ratio_lbls = []
        self.s_ratio_sliders = []

        for idx in range(4):
            row_s = ctk.CTkFrame(s_ratios_frame, fg_color="transparent")
            row_s.pack(fill="x", pady=1)
            
            ctk.CTkLabel(row_s, text=f"S{idx+1} Speed %:", font=("Tahoma", 14)).pack(side="left")
            lbl = ctk.CTkLabel(row_s, text=f"{int(self.s_speed_ratios[idx] * 100)}%", font=("Consolas", 14, "bold"))
            lbl.pack(side="right", padx=4)
            
            # บันทึกดัชนีของแต่ละปุ่ม
            slider = ctk.CTkSlider(
                row_s, from_=0.1, to=1.0, number_of_steps=18,
                command=lambda val, s_idx=idx: self._on_s_ratio_change(s_idx, val)
            )
            slider.set(self.s_speed_ratios[idx])
            slider.pack(fill="x", expand=True, padx=8)
            
            self.s_ratio_lbls.append(lbl)
            self.s_ratio_sliders.append(slider)

        # --- Sweep Tick Duration ---
        tick_row = ctk.CTkFrame(speed_frame, fg_color="transparent")
        tick_row.pack(fill="x", padx=8, pady=4)
        ctk.CTkLabel(tick_row, text="Sweep Tick (ms):", font=("Tahoma", 14)).pack(side="left")
        self.tick_lbl = ctk.CTkLabel(tick_row, text=f"{int(self.sweep_tick * 1000)}ms", font=("Consolas", 14, "bold"))
        self.tick_lbl.pack(side="right", padx=4)
        self.tick_slider = ctk.CTkSlider(
            tick_row, from_=0.001, to=0.020, number_of_steps=19,
            command=self._on_tick_change
        )
        self.tick_slider.set(self.sweep_tick)
        self.tick_slider.pack(fill="x", expand=True, padx=8)
    ...
    def _on_sweep_mode_change(self, val):
        ...
        state = "disabled" if val == "Direct (Max Speed)" else "normal"
        self.sweep_step_slider.configure(state=state)
        self.ease_chk.configure(state=state)
        self.kp_slider.configure(state=state)
        self.min_step_slider.configure(state=state)
        self.s2_kp_slider.configure(state=state)
        self.s2_min_slider.configure(state=state)
        for s in self.s_ratio_sliders:
            s.configure(state=state)
        self.tick_slider.configure(state=state)

    def _on_ease_toggle(self):
        self.use_ease = self.ease_chk_var.get()
        self._log(f"Ease-out set to: {self.use_ease}")
    ...
    def _on_s2_kp_change(self, val):
        self.s2_kp = float(val)
        self.s2_kp_lbl.configure(text=f"{self.s2_kp:.2f}")

    def _on_s2_min_step_change(self, val):
        self.s2_min_step = float(val)
        self.s2_min_lbl.configure(text=f"{self.s2_min_step:.1f}°")

    def _on_s_ratio_change(self, idx, val):
        self.s_speed_ratios[idx] = float(val)
        self.s_ratio_lbls[idx].configure(text=f"{int(self.s_speed_ratios[idx] * 100)}%")

    def _on_tick_change(self, val):
        self.sweep_tick = float(val)
        self.tick_lbl.configure(text=f"{int(self.sweep_tick * 1000)}ms")
    ...
    def _zmq_loop(self):
        while self.running:
            ...
            try:
                ...
                elif len(parts) == 3 and parts[1] == "ease":
                    use_ease = (parts[2] == "1")
                    self.use_ease = use_ease
                    self.root.after(0, lambda u=use_ease: self.ease_chk_var.set(u))
                    self._log(f"cmd: ease -> {use_ease}")
                elif len(parts) == 5:
        ...
        while self.running:
            ...
            if moving:
                ...
                for i in range(4):
                    ...
                    # ดึงค่าจำกัดความเร็วเฉพาะตัวของแต่ละ Servo
                    limit_step = self.sweep_step * self.s_speed_ratios[i]

                    if self.use_ease:
                        # คำนวณ step แบบ Ease-out: ดึงค่าปรับแต่งจาก GUI
                        kp = self.kp
                        min_step = self.min_step
                        if i == 1:
                            # สำหรับ Servo 2 ปรับสเกลลง 70% จากสไลเดอร์หลักเพื่อเซฟแรงกระแทกพื้น
                            kp = self.kp * 0.7
                            min_step = self.min_step * 0.5
                        current_step = max(min_step, min(limit_step, abs_diff * kp))
                    else:
                        # หากไม่ได้ใช้ Ease-out ให้ก้าวเคลื่อนที่ด้วยความเร็วคงที่ limit_step ตลอดทาง
